...
def get_filtered_data(df: pd.DataFrame, selected_country: str, selected_year: str) -> pd.DataFrame:
...
def get_kpis(df: pd.DataFrame):
    ...
    worst_city = df.loc[worst_city_idx, 'City'] if not pd.isna(worst_city_idx) else "N/A"
    
    most_common_source = df['Pollution_Source'].mode()[0] if not df['Pollution_Source'].empty else "N/A"
